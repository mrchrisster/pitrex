    ...
    for (;;)
    {
      //handleUARTInterface();
          ...
          vecx_direct(10000);
            ...
            {
              if ((vectrexButtonState & 0x0f) == 0x00) 
              {
                vectrexResetCount++;
...
void loadVectrexBin(char *selectedName, uint8_t *loadMem)
{
  ...
  else
  {
    ...
    if (fsize <32768*2)
    {
        printf("Matching all 4 (64k) banks to first 32k bank\r\n");
    ...
    if (fsize <32768*4)
    {
        printf("Matching first 2 (32k) banks to last two 64k bank\r\n");
        // fill all banks with the loaded ROM
        for (int r=0;r<32768;r++)
        {
          // the first 32k will be the first 32k of bank 2 (64k bank 2 that is)
          loadMem[r+32768*4] = loadMem[r+32768*0];
 
          // the second 32k will be the first 32k of bank 3 (64k bank 2 that is)
          loadMem[r+32768*6] = loadMem[r+32768*1];
        }
    }
